...
def verifier_ressources(ressources, besoin):
    ...
    for ressource, quantite_requise in besoin.items():
        quantite_disponible = ressources.get(ressource, 0)
...
- si besoin = {'oxygene': 5} et cycles=3, on consomme 15 d'oxygène.
    ...
    for ressource, quantite_par_cycle in besoin.items():
        consommation_totale = quantite_par_cycle * cycles
        nouvelles[ressource] = nouvelles.get(ressource, 0) - consommation_totale
...
def generer_alertes_ressources(ressources, seuil=50):
    ...
    for ressource, stock in ressources.items():
        if stock < seuil:
            a_commander = max(0, niveau_cible - stock)  
...
def calculer_cycles_possibles(ressources, consommations):
    ...
    for activite, conso_activite in consommations.items():
        nb_cycles_possibles = float('inf')  
        for ressource, conso_par_cycle in conso_activite.items():
            if conso_par_cycle > 0:
                stock = ressources.get(ressource, 0)
                nb_cycles = stock // conso_par_cycle
                nb_cycles_possibles = min(nb_cycles_possibles, nb_cycles)
        if nb_cycles_possibles == float('inf'):
            nb_cycles_possibles = 0  
        possibles[activite] = nb_cycles_possibles
    return possibles
...
Returns:
    ...
    for ressource, manque in manques_tries:
        if manque > 0:
            cout_unitaire = COUTS_UNITAIRES.get(ressource, float('inf'))
            quantite_achetable = min(manque, budget // cout_unitaire)
            if quantite_achetable > 0:
                achats[ressource] = quantite_achetable
                budget -= quantite_achetable * cout_unitaire
