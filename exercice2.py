...
def calculer_priorite(intervention):
    ...
    score = (urgence * 2) + (duree * 1) + (1 if critique else 0) * 10
...
def estimer_temps_interventions(liste_triee):
    ...
    temps_total = sum(intervention.get('duree', 0) * 4 for intervention in liste_triee)
    temps_stats['temps_total'] = temps_total
    # TODO 2 : Calculer le temps moyen (0 si liste vide)
    if liste_triee:
        temps_stats['temps_moyen'] = temps_total / len(liste_triee)
    else:
...
def identifier_interventions_urgentes(liste, seuil=30):
    ...
    for intervention in liste:
        urgence = intervention.get('urgence', 0)
        if urgence > seuil:
            urgentes.append(intervention.get('id', None))
    return urgentes
