...
def analyser_rapport(texte, mots_cles):
    ...
    for mot_cle, score_mot in mots_cles.items():
        occurrences = mots.count(mot_cle)
...
def categoriser_rapports(rapports, mots_cles):
    ...
    for texte in rapports:
        score,mots_trouves = analyser_rapport(texte, mots_cles)
        if score >= 7:
...
def identifier_problemes(rapports_negatifs, mots_cles_negatifs):
    ...
    for mot_negatif in mots_cles_negatifs.keys():
    ...
    for element in rapports_negatifs:
        ...
        mots = [mot.strip('.,!?;:()') for mot in mots]
        for mot_negatif in mots_cles_negatifs.keys():
            occurrences = mots.count(mot_negatif)
            problemes[mot_negatif] += occurrences
...
def generer_rapport_global(categories, problemes):
    ...
    if total_scores:
        rapport['score_moyen'] = sum(total_scores) / len(total_scores)
...
def calculer_tendance(historique_scores):
    ...
    if n <= 1:
        return 'stable'
    mid = n // 2
    premiere_moitie = historique_scores[:mid]
    deuxieme_moitie = historique_scores[mid:]
    moyenne_premiere = sum(premiere_moitie) / len(premiere_moitie)
    moyenne_deuxieme = sum(deuxieme_moitie) / len(deuxieme_moitie)
    if moyenne_deuxieme > moyenne_premiere:
        return 'amelioration'
    elif moyenne_deuxieme < moyenne_premiere:
        return 'degradation'    
    else:
        return 'stable'
